...
namespace cAlgo.Robots
{
    ...
    public class FTMO_VWAP_MeanReversion_v1 : Robot
    {
        ...
        [Parameter("Entry Sigma", Group = "Setup", DefaultValue = 2.0, MinValue = 1.8, MaxValue = 2.2, Step = 0.1)]
        public double EntrySigma { get; set; }

        [Parameter("Maker Sigma", Group = "Setup", DefaultValue = 1.5, MinValue = 1.0, MaxValue = 2.0, Step = 0.1)]
        ...
        [Parameter("Runner Sigma", Group = "Setup", DefaultValue = 1.0, MinValue = 0.5, MaxValue = 2.0, Step = 0.1)]
        public double RunnerSigma { get; set; }

        [Parameter("ADX Period", Group = "Regime", DefaultValue = 14, MinValue = 10, MaxValue = 18)]
        public int AdxPeriod { get; set; }

        [Parameter("ADX Block Level", Group = "Regime", DefaultValue = 22.0, MinValue = 15.0, MaxValue = 35.0, Step = 0.5)]
        public double AdxBlockLevel { get; set; }

        [Parameter("ATR Period", Group = "Risk", DefaultValue = 14, MinValue = 5, MaxValue = 50)]
        public int AtrPeriod { get; set; }

        [Parameter("Stop ATR Mult", Group = "Risk", DefaultValue = 1.5, MinValue = 1.3, MaxValue = 1.7, Step = 0.1)]
        ...
        [Parameter("Reentry Max Bars", Group = "Setup", DefaultValue = 3, MinValue = 1, MaxValue = 6)]
        public int ReentryMaxBars { get; set; }

        [Parameter("Pending Expiry Bars", Group = "Execution", DefaultValue = 3, MinValue = 1, MaxValue = 12)]
        public int PendingExpiryBars { get; set; }

        [Parameter("Time Stop Bars", Group = "Execution", DefaultValue = 12, MinValue = 3, MaxValue = 48)]
        public int TimeStopBars { get; set; }

        [Parameter("TP1 Close %", Group = "Exit", DefaultValue = 70.0, MinValue = 50.0, MaxValue = 90.0, Step = 5.0)]
        public double Tp1ClosePercent { get; set; }

        [Parameter("Use CVD Proxy", Group = "Flow", DefaultValue = true)]
        public bool UseCvdProxy { get; set; }

        [Parameter("CVD Divergence Lookback", Group = "Flow", DefaultValue = 12, MinValue = 4, MaxValue = 48)]
        public int CvdLookback { get; set; }

        [Parameter("Max Friction / TP1", Group = "Execution", DefaultValue = 0.25, MinValue = 0.05, MaxValue = 0.50, Step = 0.01)]
        ...
        [Parameter("Spread Freeze Multiple", Group = "Execution", DefaultValue = 2.0, MinValue = 1.2, MaxValue = 5.0, Step = 0.1)]
        public double SpreadFreezeMultiple { get; set; }

        [Parameter("Latency Guard ms", Group = "Execution", DefaultValue = 250, MinValue = 50, MaxValue = 5000, Step = 50)]
        public int LatencyGuardMs { get; set; }

        [Parameter("Latency Freeze Min", Group = "Execution", DefaultValue = 5, MinValue = 1, MaxValue = 60)]
        public int LatencyFreezeMinutes { get; set; }

        [Parameter("Bot Label", Group = "Execution", DefaultValue = "FTMO_VWAP_MR_V1")]
        ...
        protected override void OnStart()
        {
            ...
            UpdateRiskDay(true);
            WarmUpSignalState();
            Print("[{0}] START | {1} | M5 VWAP mean reversion | Risk={2:F2}% | ADX<{3:F1} | 24/5 while broker market is open",
                BotLabel, SymbolName, RiskPercent, AdxBlockLevel);
            Print("[{0}] CVD note: historical cTrader bars do not expose aggressor bid/ask volume. UseCvdProxy={1} uses signed TickVolume for deterministic backtest/live parity.",
                BotLabel, UseCvdProxy);
        }
        ...
        private void ProcessBar(int index)
        {
            ...
            if (_signal != null)
            {
                ...
                else if (_signal.Stage == SignalStage.WaitingReentry)
                {
                    ...
                    else if (IsReentry(index, snapshot, _signal.Direction))
                    {
                        ...
                        Log("REENTRY", "Dir={0} Close={1} VWAP={2} Sigma={3}", _signal.Direction,
                            _m5.ClosePrices[index], snapshot.Vwap, snapshot.Sigma);
                    }
                }
            ...
            if (close < snapshot.LowerEntry && FlowConfirms(index, snapshot.SessionStartIndex, SignalDirection.Buy))
            {
                ...
                Log("EXTENSION", "Dir=Buy Close={0} Band2={1} ADX={2:F2}", close, snapshot.LowerEntry, adx);
            }
            else if (close > snapshot.UpperEntry && FlowConfirms(index, snapshot.SessionStartIndex, SignalDirection.Sell))
            {
                ...
                Log("EXTENSION", "Dir=Sell Close={0} Band2={1} ADX={2:F2}", close, snapshot.UpperEntry, adx);
            }
        }
        ...
        private void TryPlaceMakerOrder()
        {
            ...
            if (!IsFinitePositive(target1DistancePips) || frictionPips / target1DistancePips > MaxFrictionRatio)
            {
                Log("FRICTION_BLOCK", "ratio={0:F3} spread={1:F2} target1={2:F2}",
                    frictionPips / Math.Max(target1DistancePips, 0.0001), spreadPips, target1DistancePips);
            ...
            var comment = string.Format(CultureInfo.InvariantCulture,
                "VWAPMR;INITVOL={0:R};SIG={1};RE={2}", volume, _signal.ExtensionIndex, _signal.ReentryIndex);
        ...
        private void ManageOpenPositions()
        {
            ...
            foreach (var position in positions)
            {
                ...
                if (!tp1Done)
                {
                    ...
                    var be = position.TradeType == TradeType.Buy
                        ? position.EntryPrice + costPips * Symbol.PipSize
                        : position.EntryPrice - costPips * Symbol.PipSize;
                    var stop = position.ModifyStopLossPrice(be);
                    Log("TP1", "Position={0} NewVol={1} BE={2} StopModified={3}", position.Id, newVolume, be, stop.IsSuccessful);
                    continue;
                }
        ...
        private bool TryGetVwapSnapshot(int index, out VwapSnapshot snapshot)
        {
            ...
            {
                var v = Math.Max(0.0, _m5.TickVolumes[i]);
                if (v <= 0)
                    continue;
                var p = (_m5.HighPrices[i] + _m5.LowPrices[i] + _m5.ClosePrices[i]) / 3.0;
                sumV += v;
                sumPV += p * v;
                sumP2V += p * p * v;
            }
        ...
        private void ApplyDailyCircuitBreaker()
        {
            ...
            _dailyLocked = true;
            Log("DAILY_BREAKER", "Equity={0:F2} Floor={1:F2}; flattening this bot's exposure", Account.Equity, floor);
        ...
        private bool IsSpreadFrozen()
        {
            ...
            if (average <= 0)
                return false;
            if (current <= SpreadFreezeMultiple * average)
                return false;
        ...
        {
            var minute = new DateTime(Server.Time.Year, Server.Time.Month, Server.Time.Day, Server.Time.Hour, Server.Time.Minute, 0, DateTimeKind.Utc);
        ...
        private double ParseInitialVolume(string comment, double fallback)
        {
            ...
            double value;
            return double.TryParse(text, NumberStyles.Float, CultureInfo.InvariantCulture, out value) && value > 0 ? value : fallback;
        }
